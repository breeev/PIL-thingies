...
def colorshowcase(e=None):
   g=Tk()
   print('See opened window...')
   g.geometry('1164x800')
   i=0
   for u in colors:Label(g,text=u,bg=u,width=13).place(x=97*(i//800),y=i%800);i+=20
   g.mainloop()

def RGB(color):
   r=Tk()
   rgb=tuple((c//256 for c in r.winfo_rgb(color)))
   r.destroy()
   return rgb
...
palette=['red','black','gray30','gray50','gray60','gray90','white']

print('Controls: \n- "add" to add a color to the palette;\n- "del" to remove a color from the palette;\n- "help" to get a list of color names;\n- Press enter to finish.')
input('\nPress enter to continue...')

def menu():
   global palette
   print('Current palette: '+str(palette),flush=True)
   cmd=input().lower()
   cmd=None if len(cmd)==0 else cmd[0]
   if cmd=='a':
      palette.append(input('Add a color: '))
      if not palette[-1] in colors:print(palette.pop(-1).title()+' is not a valid color.\nEnter "help" to get a list of valid colors.')
      menu()
   elif cmd=='d':
      col=input('Remove a color: ')
      if col in palette:palette.remove(col)
